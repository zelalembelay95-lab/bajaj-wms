...
async function receiveOneLine(line, user, branchCode) {
  ...
  const location = await resolveReceivingLocation(part, branchCode, line.locationCode);
  if (!location) {
    return {
      oemPartNumber: line.oemPartNumber,
      sku: part.sku,
      status: line.locationCode ? "ERROR" : "NO_BIN_AVAILABLE",
      message: line.locationCode
        ? `Location ${line.locationCode} not found in this branch`
        : `No active ${part.defaultZoneType} bin with spare capacity for "${part.category}" in this branch. Provision a new bin.`,
    };
  }

  const snapshot = await receiveIntoLocation({ part, location, branchCode, qty: line.qty, userId: user._id });
