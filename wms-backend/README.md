...
uses [Resend](https://resend.com) — free tier, 3,000 emails/month, no
credit card.

1. Sign up at resend.com → **API Keys** → create one → copy it.
2. Set `RESEND_API_KEY` in Render's environment variables.
3. **Sandbox limit to know about**: until you verify your own domain in
   Resend, their default sender can only deliver to the email address on
   your Resend account — not to your actual staff's inboxes. To send to
   everyone: Resend dashboard → **Domains** → **Add Domain** → add the DNS
   records it shows you (a few minutes if your domain's already on
   Cloudflare) → set `RESEND_FROM_EMAIL` to an address on that domain,
   e.g. `Bajaj WMS <notifications@yourdomain.com>`.
4. Leave `RESEND_API_KEY` unset and the app still works fully — PO emails
   are silently skipped (logged, not an error) rather than breaking
   anything.
