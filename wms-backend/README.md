# GUZO WMS — Backend (Express + MongoDB)
...
5. **Database → Connect → Drivers** → copy the connection string, looks like:
   ...
   ```
6. Add your database name before the `?`: `.../guzo_wms?retryWrites=true...`
...
| Email | Role |
|---|---|
| `admin@guzowms.local` | Admin |
| `ceo@guzowms.local` | Executive |
| `manager@guzowms.local` | Manager (AA-MAIN branch) |
| `store@guzowms.local` | Employee (AA-MAIN branch) |
...
1. Push this folder to GitHub (part of the same `guzo-wms` repo, in `wms-backend/`).
...
5. **Create Web Service**. Render gives you a URL like `https://guzo-wms-backend.onrender.com`.
...
When anyone submits a PO, every **Admin** (company-wide) and that branch's
**Manager(s)** get an email so they know something needs approval. This
sends through a real Gmail account's own SMTP server — free, no domain,
no third-party service, works with any Gmail/Outlook/Yahoo recipient.

1. Use (or create) a Gmail account for the company to send from, e.g.
   `guzoimportexport@gmail.com`.
2. That account needs **2-Step Verification** turned on first — Google
   requires it before it'll let you create an App Password:
   **myaccount.google.com/security → 2-Step Verification → turn it on**
   (you'll verify with a phone number).
3. Once that's on: **myaccount.google.com/apppasswords** → name it
   anything (e.g. "GUZO WMS") → **Create** → Google shows a 16-character
   code, e.g. `abcd efgh ijkl mnop`.
4. In Render's environment variables, set:
   - `GMAIL_USER` = the Gmail address itself
   - `GMAIL_APP_PASSWORD` = that 16-character code, spaces removed
   - `GMAIL_FROM_NAME` (optional) = what shows as the sender name, e.g. `GUZO Import and Export`
5. Leave both unset and the app still works fully — PO emails are silently
   skipped (logged, not an error) rather than breaking anything.

**Limits worth knowing**: free Gmail accounts cap at ~500 sends/day —
irrelevant at this app's volume. This does mean the notification emails
come from a personal-style Gmail address rather than a branded
`@yourcompany.com` one; if that matters later, moving to a real domain +
a service like Resend is a small, contained change (swap
`src/utils/sendEmail.js`'s implementation, same function signature, no
...
|---|---|---|---|
| POST | `/api/auth/login` | — | Get a session token (rate-limited: 10/15min per IP) |
| POST | `/api/auth/register` | admin | Create a new staff login |
| GET | `/api/auth/me` | any | Validate the stored token on page load |
| PUT | `/api/auth/change-password` | any | Self-service password change (requires current password) |
...
| POST | `/api/orders/confirm-pick` | any | Confirm a completed pick — actually deducts the picked stock |
...
| PUT | `/api/purchase-orders/:id/receive` | admin+manager+employee | Mark goods arrived — actually adds the stock and closes the PO |
| PUT | `/api/purchase-orders/:id/cancel` | admin+manager | Cancel a PO |
| GET | `/api/users` | admin | Staff list |
| PUT | `/api/users/:id` | admin | Edit name, email, role, branch, job title |
| PUT | `/api/users/:id/reset-password` | admin | Set a new password for someone |
| PUT | `/api/users/:id/deactivate` | admin | Disable a login |
| PUT | `/api/users/:id/reactivate` | admin | Re-enable a login |
| DELETE | `/api/users/:id` | admin | Permanently delete (blocked if they have stock-movement history — deactivate instead) |
...
- **Soft-disable, not delete**, for users (`isActive: false`) — keeps `StockMovement.performedBy` references valid for the audit trail. Hard delete is available but blocked if that user has any stock-movement history.
...
- **Rate limiting**: `/api/auth/login` allows 10 attempts per 15 minutes per IP. `server.js` sets `trust proxy` since Render sits behind a reverse proxy — without it, every request would appear to come from the same IP and rate limiting would misfire.
- **Picking actually removes stock now.** `POST /api/orders/pick-list` only proposes a route — it was never meant to touch inventory. `POST /api/orders/confirm-pick` is the step that does, called when a picker taps "Complete Pick" in the UI. Each line is its own transaction, so a partial failure on one SKU doesn't lose track of what actually left the shelf on the others.
- **The PO lifecycle is now fully closed.** `SUBMITTED → APPROVED → RECEIVED` (or `CANCELLED` from either open state). `PUT /:id/receive` isn't just a status flip — it actually runs the same stock-receiving transaction as `POST /api/inventory/receive` (both call the shared `src/utils/receiveStock.js`), so marking a PO received and having the stock actually show up in inventory are the same action, not two things that can drift out of sync.
