...
import { FileText, Check, Ban, Loader2, PackageCheck } from "lucide-react";
...
const STATUS_META: Record<PurchaseOrder["status"], { label: string; className: string }> = {
  ...
  APPROVED: { label: "Approved — Awaiting Goods", className: "bg-signal-teal-dim/40 text-signal-teal" },
...
export function PurchaseOrdersView() {
  ...
  // Receiving goods is floor-level work — same tier as receiving any other shipment.
  const canReceive = user?.role === "admin" || user?.role === "manager" || user?.role === "employee";
  ...
  async function act(id: string, action: "approve" | "cancel" | "receive") {
    setActingId(id);
    setError(null);
    try {
    ...
    <div className="mx-auto max-w-4xl px-4 py-6 md:px-8 md:py-8">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-steel-400">
          {canApprove
            ? "Approve or cancel pending orders, and mark goods received when they arrive."
            : "Track reorder requests, and mark goods received when they arrive."}
        </p>
      </header>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          {orders.map((o) => {
            ...
            return (
              <div key={o._id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-graphite-700 bg-graphite-900 p-4">
                <div className="min-w-0">
                  ...
                  {o.receivedBy && (
                    <p className="mt-1 flex items-center gap-1.5 text-xs text-steel-500">
                      ...
                      {o.receivedLocationCode && <BinTag code={o.receivedLocationCode} size="sm" />}
                    </p>
                  )}
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  {canReceive && o.status === "APPROVED" && (
                    <button
                      onClick={() => act(o._id, "receive")}
                      disabled={busy}
                      className="flex items-center gap-1 rounded-md bg-signal-amber px-2.5 py-1.5 text-xs font-semibold text-graphite-950 hover:opacity-90 disabled:opacity-50"
                    >
                      {busy ? <Loader2 size={13} className="animate-spin" /> : <PackageCheck size={13} />}
                      Mark Received
                    </button>
                  )}
                </div>
