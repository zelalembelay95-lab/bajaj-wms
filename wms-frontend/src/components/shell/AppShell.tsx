import { Search, BellRing, ClipboardList, Wifi, WifiOff, LayoutDashboard, Bike, PackageSearch, Users, LogOut, FileText, Building2, KeyRound } from "lucide-react";
import type { ReactNode } from "react";
import { useAuth } from "../../context/AuthContext";
import type { UserRole } from "../../types";
import { Logo } from "../shared/Logo";

export type ViewId = "dashboard" | "matrix" | "lowstock" | "picking" | "vehicles" | "parts" | "users" | "purchaseOrders" | "branches" | "account";
    ...
    <div className="flex min-h-screen flex-col bg-graphite-950 text-paper md:flex-row">
      ...
      <aside className="hidden w-20 flex-col items-center gap-1 border-r border-graphite-700 bg-graphite-900 py-4 md:flex lg:w-56 lg:items-stretch lg:px-3">
        <div className="mb-6 flex items-center gap-2 px-2 lg:px-1">
          <Logo size={36} className="shrink-0 rounded-md" />
          <div className="hidden lg:block">
            <p className="font-display text-sm font-semibold leading-tight">GUZO WMS</p>
            <p className="text-[11px] text-steel-400">{user?.branchCode ?? "All Branches"}</p>
          </div>
        </div>
        ...
        <div className="mt-2 border-t border-graphite-700 pt-2">
          ...
          <button
            ...
            className={`flex w-full items-center justify-center gap-2 rounded-md px-2.5 py-2 text-sm font-medium transition-colors lg:justify-start lg:px-3 ${
              active === "account" ? "bg-graphite-800 text-signal-amber" : "text-steel-400 hover:bg-graphite-800 hover:text-paper"
            }`}
          >
            <KeyRound size={16} />
      ...
      <header className="flex items-center justify-between border-b border-graphite-700 bg-graphite-900 px-4 py-3 md:hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => onNavigate("account")} className="text-steel-400 hover:text-paper">
            <KeyRound size={18} />
          </button>
