...
export function PickingSlip({ onApiHealth }: { onApiHealth: (ok: boolean) => void }) {
  ...
  async function handleCompletePick() {
    ...
    setError(null);
    try {
      const result = await api.post<{ ok: boolean; results: { sku: string; locationCode: string; status: string; message?: string }[] }>(
        "/api/orders/confirm-pick",
        {
          orderId: pickList.orderId,
          items: pickList.route.map((stop) => ({ sku: stop.sku, locationCode: stop.locationCode, qtyPicked: stop.qtyToPick })),
        }
      );
      const failed = result.results.filter((r) => r.status !== "OK");
      if (failed.length > 0) {
        setError(`${failed.length} line(s) couldn't be confirmed: ${failed.map((f) => f.message).join("; ")}`);
      } else {
        setCompleted(true);
      }
    } catch (err) {
      setError(err instanceof ApiRequestError ? err.message : "Could not confirm the pick — stock was not deducted.");
    } finally {
      setConfirming(false);
    }
  }
    ...
    <div className="mx-auto max-w-2xl px-4 py-6 md:px-8 md:py-8">
      ...
      {pickList && completed && (
        <div className="mb-4 flex items-center gap-2 rounded-md border border-signal-teal-dim bg-signal-teal-dim/20 px-3 py-2.5 text-sm text-signal-teal">
          ...
          Pick confirmed — stock has been deducted for order {pickList.orderId}.
        </div>
      )}
      ...
      {pickList && route.length > 0 && !completed && (
          ...
          <div className="mx-auto flex max-w-2xl items-center gap-3">
            ...
            {allPicked ? (
              <button
                ...
                disabled={confirming}
                className="flex items-center gap-2 rounded-md bg-signal-teal px-4 py-2.5 text-sm font-semibold text-graphite-950 hover:opacity-90 disabled:opacity-60"
              >
