...
const VEHICLE_STATUSES = ["IN_TRANSIT", "IN_STOCK", "PDI_PENDING", "ALLOCATED", "DISPATCHED", "DAMAGED", "RETURNED"] as const;
              ...
              {vehicles.map((v) => (
                <tr key={v._id} className="bg-graphite-900/40">
                  ...
                  <td className="px-4 py-3">
                    {canEdit ? (
                      <StatusSelect vehicle={v} onUpdated={load} />
                    ) : (
                      <span className={`font-medium ${STATUS_COLORS[v.status] ?? "text-steel-300"}`}>{v.status.replace(/_/g, " ")}</span>
                    )}
                  </td>
...
function StatusSelect({ vehicle, onUpdated }: { vehicle: Vehicle; onUpdated: () => void }) {
  ...
  async function handleChange(newStatus: string) {
    ...
    setLocalError(null);
    try {
      await api.put(`/api/vehicles/${vehicle._id}`, { status: newStatus });
      onUpdated();
    } catch (err) {
      setLocalError(err instanceof ApiRequestError ? err.message : "Could not update status");
    } finally {
      setUpdating(false);
    }
  }
  ...
  return (
    <div>
      <select
        ...
        className={`rounded-md border border-graphite-600 bg-graphite-800 px-2 py-1 text-xs font-medium focus:border-signal-teal disabled:opacity-50 ${
          STATUS_COLORS[vehicle.status] ?? "text-steel-300"
        }`}
      >
      ...
      {localError && <p className="mt-1 text-[10px] text-signal-red">{localError}</p>}
    </div>
  );
}
