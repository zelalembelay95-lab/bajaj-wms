...
API request automatically by `src/lib/apiClient.ts`. On a 401 from any
request, the app logs the user out automatically rather than leaving them
stuck on a broken screen.

Four roles come back on the user object: `admin`, `executive`, `manager`,
`employee` — plus an optional `branchCode` (null for admin/executive, who
are company-wide) and a free-text `jobTitle` for display (e.g. "CEO",
"Warehouse Manager"). `AppShell` filters navigation by role — Spare Parts
catalog, Branches, and Staff Logins are admin-only; Dashboard gets a
branch switcher for admin/executive; everything else follows the
permission table in the backend README.
...
src/
  ...
  components/
    ...
    dashboard/Dashboard.tsx        Post-login landing page (stats, activity, branch switcher)
    inventory/InventoryMatrix.tsx  View — Compatibility Matrix
    lowstock/LowStockCommandCenter.tsx  View — Low-Stock alerts + PO generation
    picking/PickingSlip.tsx         View — Digital Picking Slip
    purchaseorders/PurchaseOrdersView.tsx  View — PO list with Manager/Admin approve/cancel
    admin/                          Vehicles / Spare Parts / Branches / Staff Logins (mostly admin-only)
